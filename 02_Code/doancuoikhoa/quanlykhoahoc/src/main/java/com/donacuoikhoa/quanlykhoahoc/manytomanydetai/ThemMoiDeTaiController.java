...
public class ThemMoiDeTaiController {
    ...
    public String luDeTai(@Valid @ModelAttribute("detai") DeTai deTai, BindingResult bindingResult,
        ...
        if (bindingResult.hasErrors()) {
            bindingResult.getFieldErrors().forEach(error -> red.addFlashAttribute(error.getField() + "Error", error.getDefaultMessage()));
            ...
            if (canBoChuTri != null && canBoThamGia != null) {
                for (String chuTriId : canBoChuTri) {
                    if (Arrays.asList(canBoThamGia).contains(chuTriId)) {
                        red.addFlashAttribute("loi", "Tên cán bộ không được trùng nhau giữa Cán bộ chủ trì và Cán bộ tham gia.");
...
//        }
    ...
    private void saveOrUpdateQuanLyDeTai(String[] canBoList, String[] quanLyDeTaiID, DeTai deTai, VaiTroDeTai vaiTroDeTai, float diem) {
...
//    }
//
//    private void handleValidationErrors(BindingResult bindingResult, RedirectAttributes red) {
...
//                        throw new IllegalArgumentException("Tên cán bộ không được trùng nhau giữa Cán bộ chủ trì và Cán bộ tham gia.");
//                    }
//                }
//            }
//        }
//
//        if (canBoChuTri == null || canBoChuTri.length == 0) {
//            throw new IllegalArgumentException("Bạn cần phải chọn ít nhất một cán bộ chủ trì.");
//        }
//
//        for (String chuTriId : canBoChuTri) {
//            int nguoiDungId = Integer.parseInt(chuTriId);
